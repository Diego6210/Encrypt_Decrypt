using System;
using System.Security.Cryptography;
using System.IO;
using System.Text;

namespace Proyecto_Encryption
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.Clear();
            Boolean run = false;
            while(!run){
                Console.WriteLine("Opcion 1: encriptar");
                Console.WriteLine("Opcion 2: desencriptar");
                Console.WriteLine("Opcion 3: Salir");
                int option = Convert.ToInt32(Console.ReadLine());
                Console.Clear();

                if(option == 1)
                    EncryptedMessage();
                else if(option == 2)
                    DecryptedMessage();
                else
                    run = true;
            }
        }
        static void  EncryptedMessage(){
            Console.WriteLine("Mensaje a encriptar: ");
            string message = Console.ReadLine();

            Console.WriteLine("Password: ");
            string password = Console.ReadLine();
            // Get the bytes of the string
            var bytesToBeEncrypted = Encoding.UTF8.GetBytes(message);
            var passwordBytes = Encoding.UTF8.GetBytes(password);

            // Hash the password with SHA256
            passwordBytes = SHA256.Create().ComputeHash(passwordBytes);

            var bytesEncrypted = Encrypt(bytesToBeEncrypted, passwordBytes);

            Console.WriteLine(Convert.ToBase64String(bytesEncrypted)+"\n");
            Console.WriteLine("________________________________________");
        }

        static void  DecryptedMessage(){

            Console.WriteLine("Mensaje a desencriptar: ");
            string message = Console.ReadLine();

            Console.WriteLine("Password: ");
            string password = Console.ReadLine();
            // Get the bytes of the string
            var bytesToBeDecrypted = Convert.FromBase64String(message);
            var passwordBytes = Encoding.UTF8.GetBytes(password);

            passwordBytes = SHA256.Create().ComputeHash(passwordBytes);

            var bytesDecrypted = Decrypt(bytesToBeDecrypted, passwordBytes);

            Console.WriteLine(Encoding.UTF8.GetString(bytesDecrypted)+"\n");
            Console.WriteLine("________________________________________");

        }
        
        private static byte[] Encrypt(byte[] bytesToBeEncrypted, byte[] passwordBytes)
        {
            byte[] encryptedBytes = null;

            // Set your salt here, change it to meet your flavor:
            // The salt bytes must be at least 8 bytes.
            var saltBytes = new byte[] { 1, 2, 3, 4, 5, 6, 7, 8 };

            using (MemoryStream ms = new MemoryStream())
            {
                using (RijndaelManaged AES = new RijndaelManaged())
                {
                    var key = new Rfc2898DeriveBytes(passwordBytes, saltBytes, 1000);

                    AES.KeySize = 256;
                    AES.BlockSize = 128;
                    AES.Key = key.GetBytes(AES.KeySize / 8);
                    AES.IV = key.GetBytes(AES.BlockSize / 8);
                    AES.Mode = CipherMode.CBC;

                    using (var cs = new CryptoStream(ms, AES.CreateEncryptor(), CryptoStreamMode.Write))
                    {
                        cs.Write(bytesToBeEncrypted, 0, bytesToBeEncrypted.Length);
                        cs.Close();
                    }
                    encryptedBytes = ms.ToArray();
                }
            }

            return encryptedBytes;
        }

         private static byte[] Decrypt(byte[] bytesToBeDecrypted, byte[] passwordBytes)
        {
            byte[] decryptedBytes = null;

            // Set your salt here, change it to meet your flavor:
            // The salt bytes must be at least 8 bytes.
            var saltBytes = new byte[] { 1, 2, 3, 4, 5, 6, 7, 8 };

            using (MemoryStream ms = new MemoryStream())
            {
                using (RijndaelManaged AES = new RijndaelManaged())
                {
                    var key = new Rfc2898DeriveBytes(passwordBytes, saltBytes, 1000);

                    AES.KeySize = 256;
                    AES.BlockSize = 128;
                    AES.Key = key.GetBytes(AES.KeySize / 8);
                    AES.IV = key.GetBytes(AES.BlockSize / 8);
                    AES.Mode = CipherMode.CBC;

                    using (var cs = new CryptoStream(ms, AES.CreateDecryptor(), CryptoStreamMode.Write))
                    {
                        cs.Write(bytesToBeDecrypted, 0, bytesToBeDecrypted.Length);
                        cs.Close();
                    }
                    decryptedBytes = ms.ToArray();
                }
            }
            return decryptedBytes;
        }

    }
}
